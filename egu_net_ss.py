...
def initialize_parameters(seed=1):
    ...
    xavier_init = tf.contrib.layers.xavier_initializer_conv2d(seed=seed)
    zero_init = tf.zeros_initializer()

    x_w1 = tf.get_variable("x_w1", [1,1,224,256], dtype=tf.float32, initializer=xavier_init)
    x_b1 = tf.get_variable("x_b1", [256], initializer=zero_init)

    x_w2 = tf.get_variable("x_w2", [1,1,256,128], dtype=tf.float32, initializer=xavier_init)
    x_b2 = tf.get_variable("x_b2", [128], initializer=zero_init)

    x1_conv_w1 = tf.get_variable("x1_conv_w1", [5,5,224,256], dtype=tf.float32, initializer=xavier_init)
    x1_conv_b1 = tf.get_variable("x1_conv_b1", [256], initializer=zero_init)

    x1_conv_w2 = tf.get_variable("x1_conv_w2", [3,3,256,128], dtype=tf.float32, initializer=xavier_init)
    x1_conv_b2 = tf.get_variable("x1_conv_b2", [128], initializer=zero_init)

    x_w3 = tf.get_variable("x_w3", [1,1,128,32], dtype=tf.float32, initializer=xavier_init)
    x_b3 = tf.get_variable("x_b3", [32], initializer=zero_init)

    x1_conv_w4 = tf.get_variable("x1_conv_w4", [1,1,5,32], dtype=tf.float32, initializer=xavier_init)
    x1_conv_b4 = tf.get_variable("x1_conv_b4", [5], initializer=zero_init)

    x_w4 = tf.get_variable("x_w4", [1,1,32,5], dtype=tf.float32, initializer=xavier_init)
    x_b4 = tf.get_variable("x_b4", [5], initializer=zero_init)

    x_dew1 = tf.get_variable("x_dew1", [1,1,32,5], dtype=tf.float32, initializer=xavier_init)
    x_deb1 = tf.get_variable("x_deb1", [32], initializer=zero_init)

    x_dew2 = tf.get_variable("x_dew2", [1,1,128,32], dtype=tf.float32, initializer=xavier_init)
    x_deb2 = tf.get_variable("x_deb2", [128], initializer=zero_init)

    x_dew3 = tf.get_variable("x_dew3", [3,3,256,128], dtype=tf.float32, initializer=xavier_init)
    x_deb3 = tf.get_variable("x_deb3", [256], initializer=zero_init)

    x_dew4 = tf.get_variable("x_dew4", [5,5,224,256], dtype=tf.float32, initializer=xavier_init)
    x_deb4 = tf.get_variable("x_deb4", [224], initializer=zero_init)
...
def model(x_pure, x_mixed, parameters, isTraining, keep_prob, momentum=0.9):
...
def train(x_pure_set, x_mixed_set, x_mixed_set1, y_train, y_test, learning_rate_base=0.1, beta_reg=0.005, num_epochs=200, minibatch_size=8000, print_cost=True, seed=1):
    ...
    with tf.name_scope("network"):
         x_pure_layer, x_mixed_layer, x_mixed_de_layer, l2_loss, abundances_pure = model(x_train_pure, x_train_mixed, parameters, isTraining, keep_prob)

    global_step = tf.Variable(0, trainable=False)
    learning_rate = tf.train.exponential_decay(learning_rate_base, global_step, m/minibatch_size, 0.99)

    with tf.name_scope("optimization"):
         cost, optimizer = optimize(x_pure_layer, y, x_mixed_de_layer, x_train_mixed, l2_loss, beta_reg, learning_rate, global_step)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            abund, epoch_cost_dev, epoch_acc_dev = sess.run(
                [abundances_pure, cost, accuracy],
                feed_dict={
            ...
            if ((print_cost) and (epoch % 50 == 0)):
                print('epoch {:d}: Train_loss: {:f}, Val_loss: {:f}, Train_acc: {:f}, Val_acc: {:f}'.format(
...
if __name__ == '__main__':
    ...
    parameters, val_acc, abund = train(Pure_TrSet, Mixed_TrSet, Mixed_TrSet, Y_train, Y_test, num_epochs=5)
